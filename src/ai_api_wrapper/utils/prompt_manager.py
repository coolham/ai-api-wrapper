...
class PromptManager:
    ...
    def __init__(self):
        logger = Logger.create_logger('prompt_manager')
        # 配置文件路径设置
    ...
    def save_custom_prompt(self, prompt_type: str, prompt_text: str) -> bool:
        ...
        try:
            ...
            logger.info(f"已保存自定义{prompt_type}提示词")
            return True
            
        except Exception as e:
            logger.error(f"保存自定义提示词失败: {str(e)}")
            return False
    ...
    def reset_custom_prompt(self, prompt_type: str) -> bool:
        ...
        try:
            ...
            logger.info(f"已重置{prompt_type}提示词")
            return True
